...
from backend.database import Database, Person, Cycle, EOMCycle, EOMNominee, EOMWinner
...
class TestRotationRuleViolations:
    ...
    def test_max_wins_per_period_violation(self, test_db, test_data):
        ...
        for case in max_wins_cases:
            ...
            assert any('max' in error.lower() or 'period' in error.lower() for error in result.errors), \
    ...
    def test_leader_limit_violation(self, test_db, test_data):
        ...
        for case in leader_limit_cases:
            if case.nominated_by == valid_nom.nominated_by:
                ...
                assert any('leader' in error.lower() or 'limit' in error.lower() for error in result.errors), \
...
class TestInvalidData:
    ...
    def test_inactive_nominee(self, test_db, test_data):
        ...
        assert any('active' in error.lower() for error in result.errors), \
            f"Expected active error but got: {result.errors}"
...
class TestBatchValidation:
    ...
    def test_batch_validation_mixed(self, test_db, test_data):
        ...
        assert any(r['is_valid'] for r in results), "Should have some valid results"
        assert any(not r['is_valid'] for r in results), "Should have some invalid results"
