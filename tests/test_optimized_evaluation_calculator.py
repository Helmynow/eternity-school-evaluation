...
class TestBulkDataLoading:
    ...
    def test_load_all_data_bulk(self, mock_db_session):
        ...
        def query_side_effect(*args):
            # The implementation may call session.query() with many columns; we only
            # need to distinguish the people query.
...
class TestVectorizedOperations:
    ...
    def test_apply_weights_vectorized(self, mock_db_session):
        ...
        people_df = pd.DataFrame(
            {"email": ["person1@eternity.edu", "person2@eternity.edu"], "role_title": ["Teacher", "Coordinator"]}
        )

        # Set staff type cache
        calculator._staff_type_cache = {"person1@eternity.edu": "academic", "person2@eternity.edu": "admin"}
