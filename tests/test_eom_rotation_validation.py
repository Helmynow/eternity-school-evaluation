...
from backend.database import Cycle, EOMCategory, EOMCycle, EOMNominee, EOMRotationRule, EOMWinner, Person
from backend.eom_rotation_manager import EOMRotationManager
from backend.eom_validation import EOMNominationValidator, RotationPeriodType, ValidationResult
...
class TestComprehensiveRotationRules:
    ...
    def test_check_comprehensive_rotation_rules_with_rule(self, mock_db_session, sample_eom_cycle, sample_rotation_rule):
        ...
        result = validator._check_comprehensive_rotation_rules(
            nominee_email="teacher1@example.com", eom_cycle_id=1, category="academic", term="2024-Q1", month=3, year=2024
        )

        assert isinstance(result, dict)
        assert "rules_applied" in result.get("details", {})

    def test_check_cooldown_period_no_previous_wins(self, mock_db_session, sample_rotation_rule):
    ...
    def test_check_cooldown_period_with_previous_win(self, mock_db_session, sample_rotation_rule):
    ...
    def test_check_max_wins_per_period(self, mock_db_session, sample_rotation_rule):
    ...
    def test_check_max_wins_per_period_exceeded(self, mock_db_session, sample_rotation_rule):
...
class TestPeriodCalculations:
    ...
    def test_calculate_period_dates_year(self, mock_db_session):
        ...
        period_start, period_end = validator._calculate_period_dates(current_month=6, current_year=2024, period_type="year")
    ...
    def test_calculate_period_dates_quarter(self, mock_db_session):
        ...
        period_start, period_end = validator._calculate_period_dates(current_month=5, current_year=2024, period_type="quarter")
    ...
    def test_calculate_period_dates_month(self, mock_db_session):
        ...
        period_start, period_end = validator._calculate_period_dates(current_month=3, current_year=2024, period_type="month")
    ...
    def test_calculate_cooldown_end_year(self, mock_db_session):
        ...
        cooldown_end = validator._calculate_cooldown_end(last_win_date=last_win, cooldown_period=1, period_type="year")
    ...
    def test_calculate_cooldown_end_quarter(self, mock_db_session):
        ...
        cooldown_end = validator._calculate_cooldown_end(
            last_win_date=last_win, cooldown_period=2, period_type="quarter"  # 2 quarters = 6 months
        )
...
class TestRotationRuleManagement:
    ...
    def test_create_rotation_rule(self, mock_db_session):
        ...
        assert isinstance(rule, EOMRotationRule)
    ...
    def test_update_rotation_rule(self, mock_db_session, sample_rotation_rule):
        ...
        updated_rule = validator.update_rotation_rule(rule_id=1, cooldown_period=6, max_wins_per_period=2)

        assert sample_rotation_rule.cooldown_period == 6
        assert sample_rotation_rule.max_wins_per_period == 2
        mock_db_session.commit.assert_called_once()

    def test_get_rotation_rules_for_cycle(self, mock_db_session, sample_rotation_rule):
        ...
        rules = validator.get_rotation_rules_for_cycle(cycle_id=1, active_only=True)
...
class TestEligibilityChecking:
    ...
    def test_check_nominee_rotation_eligibility_no_rules(self, mock_db_session, sample_eom_cycle):
        ...
        eligibility = validator.check_nominee_rotation_eligibility(
            nominee_email="teacher1@example.com", category=EOMCategory.OUTSTANDING_LEADERSHIP, eom_cycle_id=1
        )

        assert eligibility["eligible"] is True
        assert eligibility["rule_type"] == "default"

    def test_check_nominee_rotation_eligibility_with_rules(self, mock_db_session, sample_eom_cycle, sample_rotation_rule):
...
class TestRotationManager:
    ...
    def test_get_rotation_summary(self, mock_db_session):
        ...
        mock_db_session.query.side_effect = lambda model: {EOMRotationRule: rule_query, EOMCycle: eom_cycle_query}.get(
            model, MagicMock()
        )

        # Mock analytics
        manager.validator.get_rotation_analytics = Mock(return_value={"total_wins": 0, "unique_winners": 0})
...
class TestDefaultRotationRules:
    ...
    def test_check_default_rotation_rules_no_previous_win(self, mock_db_session):
        ...
        result = validator._check_default_rotation_rules(
            nominee_email="teacher1@example.com", term="2024-Q1", category="academic"
        )

        assert result["is_valid"] is True
        assert result["rule_type"] == "default"

    def test_check_default_rotation_rules_with_previous_win(self, mock_db_session):
...
class TestBatchValidation:
    ...
    def test_validate_batch_nominations(self, mock_db_session):
        ...
        # Mock validation
        validator.validate_nomination = Mock(return_value=ValidationResult(is_valid=True, errors=[], warnings=[], details={}))

        nominations = [
            {"nominee_email": "teacher1@example.com", "nominated_by": "manager@example.com", "category": "academic"},
            {"nominee_email": "teacher2@example.com", "nominated_by": "manager@example.com", "category": "admin"},
        ]

        results = validator.validate_batch_nominations(nominations=nominations, eom_cycle_id=1)
...
class TestAnalytics:
    ...
    def test_get_nominee_rotation_history(self, mock_db_session):
        ...
        mock_db_session.query.side_effect = lambda model: {EOMNominee: nominee_query, EOMWinner: winner_query}.get(
            model, MagicMock()
        )

        history = validator.get_nominee_rotation_history(
            nominee_email="teacher1@example.com", category=EOMCategory.OUTSTANDING_LEADERSHIP
        )
