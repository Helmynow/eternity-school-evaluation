...
from backend.weight_matrix_handler import WeightMatrixHandler, EvaluationScore, ValidationResult
...
class TestWeightMatrixHandlerEdgeCases:
    ...
    def test_validate_evaluations_missing_required_contexts(self, mock_db_session):
        ...
        assert isinstance(result, ValidationResult)
        assert len(result.warnings) > 0
        assert any('ceo' in warning.lower() or 'p&c' in warning.lower() or 'qa' in warning.lower() 
...
class TestBiasDetectorWeightCalculationEdgeCases:
    ...
    def test_calculate_weighted_score_extreme_weights(self, mock_db_session):
        ...
        weights = {
            'peer_review': 0.99,  # Very high weight
            'manager_review': 0.01  # Very low weight
        }
    ...
    def test_calculate_weighted_score_negative_scores(self, mock_db_session):
        ...
        scores = {
            'peer_review': -1.0,  # Negative score (shouldn't happen in practice)
            'manager_review': 3.0
        }
