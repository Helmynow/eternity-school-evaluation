...
from backend.weight_matrix_handler import ValidationResult, WeightMatrixHandler
...
def test_load_evaluations(mock_db_session, sample_evaluations, sample_assignments):
    ...
    # WeightMatrixHandler.load_evaluations uses a single join query: query(Evaluation, Assignment)
    join_query = MagicMock()
    join_query.join.return_value = join_query
    join_query.filter.return_value = join_query
    join_query.all.return_value = list(zip(sample_evaluations, sample_assignments))
