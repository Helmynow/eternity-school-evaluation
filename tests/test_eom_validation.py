...
from backend.database import EOMCycle, EOMNominee, EOMWinner, Person, Attendance, Cycle
...
def test_validate_nomination_rotation_rules(mock_db_session, sample_eom_cycle):
    ...
    result = validator.validate_nomination(
        ...
        eom_cycle_id=1,
        nominated_by='nominator@example.com',
        category='academic'
    )
...
def test_check_duplicate_nominations(mock_db_session):
    ...
    result = validator._check_duplicate_nominations(
        'nominee@example.com',
        1,
        'academic'
    )
...
def test_check_leader_nomination_limit(mock_db_session):
    ...
    mock_db_session.query.side_effect = lambda model: (
        person_query if model == Person else nominee_query
    )
    
    result = validator._check_leader_nomination_limit(
        'leader@example.com',
        1,
        'academic'
    )
...
def test_validate_attendance(mock_db_session):
    ...
    result = validator._validate_attendance(
        'nominee@example.com',
        3,
        2024,
        None
