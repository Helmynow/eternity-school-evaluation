...
class ContextAware360BiasDetection:
    ...
    def generate_context_aware_report(self, cycle_id: int) -> Dict:
        ...
        if not context_data:
            return {
                'status': 'no_data',
                'message': 'No evaluation data found for this cycle'
            }
        ...
        recommendations = self._generate_context_aware_recommendations(
            all_findings, cross_context_analyses, context_data
        )
    ...
    def _load_context_data(self, cycle_id: int) -> Dict[str, Dict]:
        ...
        for eval_obj, assignment in evaluations_and_assignments:
            ...
            context_data[context]['rater_target_pairs'].append({
                'rater': assignment.rater_email,
                'target': assignment.target_email,
    ...
    def _analyze_context_specific_bias(
        ...
        if std_rating < expected_std * 0.5:
            findings.append(ContextBiasFinding(
                context=context,
    ...
    def _analyze_cross_context_comparisons(
        self,
        context_data: Dict[str, Dict]
    ) -> List[CrossContextAnalysis]:
        ...
        for i, context1 in enumerate(contexts):
            for context2 in contexts[i+1:]:
                ...
                pooled_std = np.sqrt(
                    (np.var(ratings1) + np.var(ratings2)) / 2
                )
                ...
                analyses.append(CrossContextAnalysis(
    ...
    def _detect_context_patterns(
        ...
        for context, data in context_data.items():
            if context in self.CONTEXT_HIERARCHY and len(data['ratings']) >= 3:
                hierarchy_means[context] = {
                    'mean': np.mean(data['ratings']),
                    'hierarchy': self.CONTEXT_HIERARCHY[context]
                }
    ...
    def _analyze_context_balance(
        ...
        # Check for missing required contexts
    ...
    def _multi_context_statistical_analysis(
        self,
        context_data: Dict[str, Dict]
    ) -> List[BiasFinding]:
        ...
        for target, contexts in target_context_coverage.items():
            ...
            for context, data in context_data.items():
                if target in data['targets']:
                    count = sum(
                        1 for pair in data['rater_target_pairs']
            ...
            for context, min_count in self.MIN_EVALUATIONS_PER_CONTEXT.items():
                if context in contexts and context_counts.get(context, 0) < min_count:
                    insufficient.append({
                        'context': context,
                        'current': context_counts.get(context, 0),
                        'required': min_count
                    })
    ...
    def _calculate_context_bias_scores(
        self,
        context_findings: List[ContextBiasFinding]
    ) -> Dict[str, float]:
        ...
        return {
            context: float(np.mean(scores)) if scores else 0.0
            for context, scores in context_scores.items()
        }
    
    def _get_context_coverage_summary(
        self,
        context_data: Dict[str, Dict]
    ) -> Dict:
    ...
    def _get_multi_context_statistics(
        self,
        context_data: Dict[str, Dict]
    ) -> Dict:
    ...
    def _generate_context_aware_recommendations(
        ...
        cross_context_analyses: List[CrossContextAnalysis],
        context_data: Dict[str, Dict]
    ) -> List[str]:
        ...
        if high_bias_contexts:
            recommendations.append(
                f"Priority: Provide calibration training for contexts: {', '.join(high_bias_contexts)}"
            )
        
        # Cross-context recommendations
        high_bias_pairs = [
            analysis for analysis in cross_context_analyses
            if analysis.bias_indication in ['high', 'medium']
        ]
        
        if high_bias_pairs:
            recommendations.append(
                f"Review rating differences between {len(high_bias_pairs)} context pairs showing bias"
            )
        
        # Coverage recommendations
        if finding_types.get('insufficient_context_coverage', 0) > 0:
            recommendations.append(
                "CRITICAL: Ensure complete 360-degree coverage for all targets"
            )
        
        if finding_types.get('missing_required_contexts', 0) > 0:
            recommendations.append(
                "CRITICAL: Include all required rater contexts in evaluation cycle"
            )
        
        # Pattern-based recommendations
        if finding_types.get('hierarchy_bias', 0) > 0:
            recommendations.append(
                "Implement measures to reduce hierarchy-based rating bias"
            )
        ...
        if finding_types.get('context_inconsistency', 0) > 0:
            recommendations.append(
                "Conduct cross-context calibration sessions to improve consistency"
            )
        
        if not recommendations:
            recommendations.append(
                "No significant context-specific bias detected. Continue monitoring."
            )
        
        return recommendations
    
    def get_target_context_analysis(
        self,
        cycle_id: int,
        target_email: str
    ) -> Dict:
