...
from backend.database import Person, Assignment, Cycle, WeightMatrix
from backend.academic_admin_scoring import AcademicAdminScoring
from backend.audit_logger import AuditLogger, ActionType as DBActionType
import logging
...
class EvaluatorAssignmentManager:
        ...
        if staff_type.lower() == 'academic':
            return [
                ...
                },
                {
                    ...
                    'weight': 0.25,
                    'required': True,
                    'description': 'People & Culture (25%)'
                },
                {
                    'rater_context': 'coordinator_hod',
                    'weight': 0.25,
                    'required': True,
                    'description': 'Coordinator/HOD (25%)'
                },
                {
                    'rater_context': 'CEO',
                    'weight': 0.15,
                    'required': True,
                    'description': 'Director/CEO (15%)'
                },
                {
                    'rater_context': 'self_review',
                    'weight': 0.05,
                    'required': True,
                    'description': 'Self Evaluation (5%)'
                },
                {
            ...
            return [
                ...
                },
                {
                    ...
                    'weight': 0.10,
                    'required': True,
                    'description': 'Quality Assurance (10%)'
                },
                {
                    'rater_context': 'CEO',
                    'weight': 0.15,
    ...
    def find_evaluator_by_context(
        ...
        if rater_context == 'CEO':
            # Find CEO
            ceo = self.db.query(Person).filter(
        ...
        if rater_context == 'P&C':
            # Find P&C person
            pnc = self.db.query(Person).filter(
        ...
        if rater_context == 'manager_review':
            ...
            if staff_type == 'academic':
                ...
                manager = self.db.query(Person).filter(
        ...
        if rater_context == 'coordinator_hod':
            ...
            coordinator = self.db.query(Person).filter(
        ...
        if rater_context == 'QA':
            ...
            qa = self.db.query(Person).filter(
    ...
    def create_assignments_for_staff(
        ...
        target_email: str,
        cycle_id: int,
        evaluator_overrides: Optional[Dict[str, str]] = None
    ) -> List[Assignment]:
        """
            ...
            if evaluator_overrides and rater_context in evaluator_overrides:
                ...
                if not rater_person:
                    if eval_config.get('required', False):
                        self.logger.warning(
                            f"Required evaluator not found for {target_email}: {rater_context}"
                        )
                    continue
