...
class OptimizedEvaluationCalculator:
    ...
    def __init__(self, db_session):
        ...
        self._weight_matrix_cache = {
            'academic': self.ACADEMIC_WEIGHT_MATRIX,
            'admin': self.ADMIN_WEIGHT_MATRIX
        }
    ...
    def _apply_weights_vectorized(
        ...
        staff_type_map = {
            email: self._staff_type_cache.get(email, 'academic')
            for email in evaluations_df['target_email'].unique()
        }
    ...
    def calculate_batch_scores_optimized(
        ...
        if staff_type:
            evaluations_df = evaluations_df[
                evaluations_df['staff_type'].str.lower() == staff_type.lower()
            ]
        ...
        for target_email, group_df in grouped:
            ...
            for context, context_df in context_grouped:
                ...
                context_breakdown[context] = {
                    ...
                    'weighted_average': float(np.sum(context_weighted) / np.sum(context_weights)) if np.sum(context_weights) > 0 else 0.0,
    ...
    def calculate_single_score_optimized(
        ...
        """
        results = self.calculate_batch_scores_optimized(
            cycle_id=cycle_id,
    ...
    def get_score_statistics(
        ...
        """
        scores = self.calculate_batch_scores_optimized(
            cycle_id=cycle_id,
            staff_type=staff_type
        )
    ...
    def compare_academic_vs_admin_optimized(
        self,
        cycle_id: int
    ) -> Dict:
        """
