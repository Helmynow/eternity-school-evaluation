...
class TaskScheduler:
    ...
    def schedule_daily_tasks(self):
        ...
        self.scheduler.add_job(
            self.process_pending_emails, trigger=IntervalTrigger(minutes=5), id="process_pending_emails", replace_existing=True
        )
    ...
    def send_daily_smart_reminders(self):
        ...
        try:
            with get_db_session() as db:
                ...
                active_cycles = db.query(Cycle).filter(Cycle.is_active == True).all()
                        ...
                        for assignment in overdue:
                            ...
                            if days_overdue > 0:
                                ...
                                notification_system.send_notification(
                                    ...
                                    context={
                                        "target_name": (
                                            assignment.target.full_name if assignment.target else assignment.target_email
                                        ),
            ...
            with get_db_session() as db:
                ...
                for cycle in active_cycles:
                    ...
                    if 1 <= days_until_deadline <= 3:
                        ...
                        pending = (
                            db.query(Assignment)
                            .filter(Assignment.cycle_id == cycle.id)
                            .join(Evaluation, isouter=True)
        ...
        try:
            with get_db_session() as db:
                ...
                pending_emails = (
                    ...
                    .filter(EmailNotification.status == "pending", EmailNotification.sent_at <= datetime.utcnow())
                    .limit(50)
                    .all()
                )
                    ...
                    try:
                        ...
                        notification_system.send_notification(
                            ...
                            context={
                                ...
                                "cycle_name": (
                                    db.query(Cycle).filter(Cycle.id == cycle_id).first().name
                                    if db.query(Cycle).filter(Cycle.id == cycle_id).first()
                                    else "Current Cycle"
                                ),
                                "deadline": (
                                    db.query(Cycle).filter(Cycle.id == cycle_id).first().end_date.isoformat()
                                    if db.query(Cycle).filter(Cycle.id == cycle_id).first()
                                    and db.query(Cycle).filter(Cycle.id == cycle_id).first().end_date
                                    else None
                                ),
                            },
                            priority="normal",
