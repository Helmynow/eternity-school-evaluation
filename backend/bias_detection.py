...
class BiasDetector:
    ...
    def detect_recency_bias(self, cycle_id: int) -> Dict:
        ...
        for eval in evaluations:
            ...
            if assignment and eval.submitted_at:
                days = (eval.submitted_at.date() - cycle.start_date).days if hasattr(eval.submitted_at, 'date') else (eval.submitted_at - cycle.start_date).days
        ...
        return {
            ...
            'total_evaluations': len(evaluations),
            'message': 'Positive correlation suggests later submissions rate higher (recency bias)' if correlation > 0.3 
                      else 'Negative correlation suggests later submissions rate lower' if correlation < -0.3
    ...
    def detect_centrality_bias(self, cycle_id: int) -> Dict:
        ...
        results = {
            ...
            'interpretation': 'low' if std_rating < 0.5 * expected_std else 'moderate' if std_rating < 0.8 * expected_std else 'normal'
        }
    ...
    def detect_harshness_bias(self, cycle_id: int) -> Dict:
        ...
        for rater_email, ratings in ratings_by_rater.items():
            if len(ratings) >= 3:  # Need multiple ratings
                ...
                rated_targets = [assignment.target_email for eval in evaluations 
                                for assignment in [self.db.query(Assignment).filter(
                                    Assignment.id == eval.assignment_id).first()]
    ...
    def calculate_weighted_score(self, scores: Dict, weights: Dict) -> float:
        ...
        for domain, score in scores.items():
            ...
            if domain not in weights and len(weights) > 0:
                self.logger.warning(
                    f"Weight not found for domain '{domain}', using default weight of 1.0"
                )
    ...
    def detect_evaluation_bias(self, evaluations: pd.DataFrame) -> Dict:
        ...
        bias_report = {
            ...
            'recency_bias': self._detect_recency_bias_from_df(df) if 'submitted_at' in df.columns else {
                'status': 'no_timestamp_data',
                'message': 'submitted_at column not available for recency analysis'
            },
    ...
    def _detect_department_bias_from_df(self, df: pd.DataFrame) -> Dict:
        ...
        if dept_col is None:
            return {
                'status': 'no_department_data',
                'message': 'No department/segment column found in data'
            }
            ...
            if len(dept_groups) >= 2:
                try:
                    ...
                    statistical_test = {
                        ...
                        'p_value': float(p_value),
                        'significant': p_value < 0.05,
                        'interpretation': 'Significant difference between departments' if p_value < 0.05 
        ...
        return {
            ...
            'message': f'Analyzed {len(dept_stats)} departments. ' +
                      (f'Found {len(deviations)} departments with significant deviations.' if deviations 
    ...
    def _calculate_rater_reliability_from_df(self, df: pd.DataFrame) -> Dict:
        ...
        if 'target_id' not in df.columns or 'rater_id' not in df.columns:
            return {
                'status': 'invalid_data',
                'message': 'target_id and rater_id columns required for reliability analysis'
            }
        ...
        cvs = [t['coefficient_of_variation'] for t in target_reliability 
               if t['coefficient_of_variation'] is not None]
        ...
        return {
            ...
            'interpretation': 'High reliability' if avg_cv and avg_cv < 0.15 
                             else 'Moderate reliability' if avg_cv and avg_cv < 0.25
