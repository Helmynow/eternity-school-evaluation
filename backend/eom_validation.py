...
from backend.database import (
    EOMNominee, EOMCycle, EOMWinner, EOMVoter, Person, Attendance, Cycle,
    EOMRotationRule, EOMCategory
)
...
class EOMNominationValidator:
    ...
    def validate_nomination(
        ...
        if check_attendance:
            attendance_check = self._validate_attendance(
                nominee_email, eom_cycle.month, eom_cycle.year, cycle
            )
        ...
        duplicate_check = self._check_duplicate_nominations(
            nominee_email, eom_cycle_id, category
        )
        ...
        leader_limit_check = self._check_leader_nomination_limit(
            nominated_by, eom_cycle_id, category
        )
        ...
        additional_checks = self._additional_validations(
            nominee_email, nominated_by, eom_cycle_id
        )
    ...
    def validate_batch_nominations(
        self,
    ...
    def _check_comprehensive_rotation_rules(
        ...
        nominee_record = self.db.query(EOMNominee).filter(
            EOMNominee.nominee_email == nominee_email
        ).order_by(EOMNominee.created_at.desc()).first()
        
        if nominee_record and not nominee_record.rotation_eligible:
            errors.append(
                f"{nominee_email} is not eligible for nomination "
                "(rotation_eligible flag is False)"
            )
    ...
    def _get_rotation_rules(
        ...
        if isinstance(category, str):
            try:
                category_enum = EOMCategory[category.upper()]
        ...
        rules = self.db.query(EOMRotationRule).filter(
    ...
    def _check_cooldown_period(
        ...
        if not last_win:
            ...
            return {
                'is_valid': True,
                'message': 'No previous wins, cooldown does not apply'
            }
        
        # Calculate cooldown period based on period_type
        cooldown_end = self._calculate_cooldown_end(
        ...
        if current_date < cooldown_end:
            days_remaining = (cooldown_end - current_date).days
            return {
                'is_valid': False,
                'errors': [
                    f"{nominee_email} is still in cooldown period. "
                    f"Cooldown ends on {cooldown_end.strftime('%Y-%m-%d')} "
                    f"({days_remaining} days remaining). "
                    f"Last win: {last_win.announced_at.strftime('%Y-%m-%d')}"
                ],
                'last_win_date': last_win.announced_at.isoformat(),
                'cooldown_end_date': cooldown_end.isoformat(),
    ...
    def _calculate_cooldown_end(
        ...
        if period_type_str == RotationPeriodType.YEAR.value or period_type_str == 'year':
            return date(last_win_date.year + cooldown_period, last_win_date.month, last_win_date.day)
            ...
            while new_month > 12:
                new_month -= 12
                new_year += 1
            return date(new_year, new_month, last_win_date.day)
    ...
    def _check_default_rotation_rules(
        self,
        nominee_email: str,
        term: str,
        category: Optional[str]
        ...
        if existing_winner:
            return {
                'is_valid': False,
                'errors': [
                    f"{nominee_email} has already won EOM in term {term}"
        ...
        recent_wins = self.db.query(EOMWinner).filter(
            EOMWinner.winner_email == nominee_email
        ).order_by(EOMWinner.announced_at.desc()).limit(3).all()
        
        warnings = []
        if recent_wins:
            warnings.append(
                f"{nominee_email} has won EOM {len(recent_wins)} time(s) recently. "
    ...
    def _validate_attendance(
        ...
        attendance_records = self.db.query(Attendance).filter(
            Attendance.person_email == nominee_email,
        ...
        if not attendance_records:
            return {
                'is_valid': False,
                'errors': [
                    f"No attendance records found for {nominee_email} "
                    f"in {year}-{month:02d}"
                ],
                'attendance_count': 0
            }
        ...
        if absent_days > total_days * 0.1:
            warnings.append(
                f"{nominee_email} has {absent_days} absent days out of {total_days} "
                f"({absent_days/total_days:.1%})"
            )
        
        # Warning for frequent lateness
        if late_days > total_days * 0.15:
            warnings.append(
                f"{nominee_email} has {late_days} late days ({late_days/total_days:.1%})"
            )
    ...
    def _check_duplicate_nominations(
        self,
        nominee_email: str,
        eom_cycle_id: int,
        category: Optional[str] = None
    ...
    def _check_leader_nomination_limit(
        self,
        nominated_by: str,
        eom_cycle_id: int,
        category: Optional[str] = None
        ...
        if not person:
            return {
                'is_valid': False,
                'errors': [f"Person {nominated_by} not found"]
            }
        ...
        if person.role_title:
            ...
            is_leader = any(
                keyword.lower() in person.role_title.lower()
    ...
    def _additional_validations(
        ...
        if nominee_email == nominated_by:
            warnings.append("Person is nominating themselves (self-nomination)")
        ...
        if eom_cycle:
            is_voter = self.db.query(EOMVoter).filter(
                EOMVoter.eom_cycle_id == eom_cycle_id,
                EOMVoter.voter_email == nominee_email
            ).first()
            
            if is_voter:
                warnings.append(
                    f"{nominee_email} is also a voter in this EOM cycle"
                )
    ...
    def create_rotation_rule(
        ...
        self.logger.info(
            f"Created rotation rule for category {category.value} "
            f"in cycle {cycle_id}"
        )
    ...
    def update_rotation_rule(
        ...
        """
        rule = self.db.query(EOMRotationRule).filter(
            EOMRotationRule.id == rule_id
        ).first()
    ...
    def get_rotation_rules_for_cycle(
        ...
        """
        query = self.db.query(EOMRotationRule).filter(
            EOMRotationRule.cycle_id == cycle_id
        )
    ...
    def check_nominee_rotation_eligibility(
        self,
        nominee_email: str,
        category: EOMCategory,
        eom_cycle_id: int
        ...
        if not eom_cycle:
            return {
                'eligible': False,
                'reason': f"EOM cycle {eom_cycle_id} not found"
            }
        ...
        if not rules:
            ...
            if nominee_record and not nominee_record.rotation_eligible:
                return {
                    'eligible': False,
                    'reason': 'rotation_eligible flag is False',
                    'rule_type': 'default'
                }
            
            return {
                'eligible': True,
                'reason': 'No rotation rules configured',
                'rule_type': 'default'
            }
        ...
        for rule in rules:
            period_start, period_end = self._calculate_period_dates(
                eom_cycle.month, eom_cycle.year, rule.period_type
            )
            
            # Check cooldown
            last_win = self.db.query(EOMWinner).filter(
                EOMWinner.winner_email == nominee_email,
                EOMWinner.category == category.value
            ).order_by(EOMWinner.announced_at.desc()).first()
            
            cooldown_passed = True
            if last_win:
                cooldown_end = self._calculate_cooldown_end(
                    last_win.announced_at, rule.cooldown_period, rule.period_type
                )
                current_date = date(eom_cycle.year, eom_cycle.month, 1)
                cooldown_passed = current_date >= cooldown_end
            
            # Check max wins
            wins_in_period = self.db.query(EOMWinner).filter(
                EOMWinner.winner_email == nominee_email,
                EOMWinner.category == category.value,
                EOMWinner.announced_at >= period_start,
                EOMWinner.announced_at <= period_end
    ...
    def get_rotation_analytics(
        ...
        # Find repeat winners
        repeat_winners = {
            email: count for email, count in wins_by_winner.items() if count > 1
        }
        ...
        if rotation_rules:
            for rule in rotation_rules:
                ...
                for winner in winners:
                    ...
                    period_start, period_end = self._calculate_period_dates(
                        winner.announced_at.month if hasattr(winner.announced_at, 'month') 
                        else winner.announced_at.month,
    ...
    def get_nominee_rotation_history(
        self,
        nominee_email: str,
        category: Optional[EOMCategory] = None
    ) -> Dict:
        ...
        if category:
            ...
            latest_cycle = self.db.query(EOMCycle).order_by(
                EOMCycle.year.desc(), EOMCycle.month.desc()
            ).first()
            
            if latest_cycle:
                current_eligibility = self.check_nominee_rotation_eligibility(
                    nominee_email, category, latest_cycle.id
                )
