...
class SurveyIdentityManager:
    ...
    def set_identity_preference(self, user_id: str, preference: str, survey_id: Optional[int] = None) -> Dict:
        """
    ...
    def get_reveal_options(self, preference: str) -> Dict:
        ...
        if mode == IdentityMode.ANONYMOUS:
            ...
            return {
                "can_reveal": True,
                "options": [
                    {"method": RevealMethod.FULL.value, "description": "Full identity reveal", "available": True},
                    {"method": RevealMethod.PARTIAL_ROLE.value, "description": "Reveal role only", "available": True},
                    {
            ...
            return {
                ...
                "options": [
                    ...
                    {"method": RevealMethod.GRADUAL.value, "description": "Gradual reveal over time", "available": True},
                    {"method": RevealMethod.PARTIAL_ROLE.value, "description": "Reveal role only", "available": True},
                ],
    ...
    def handle_identity_reveal(self, user_id: str, reveal_request: Dict, survey_id: Optional[int] = None) -> Dict:
        """
        ...
        try:
            ...
            # Execute reveal
            reveal_result = self._execute_reveal(user_id, reveal_method, reveal_request, survey_id)
    ...
    def verify_reveal_conditions(self, user_id: str, reveal_request: Dict, survey_id: Optional[int] = None) -> Dict:
        """
        ...
        if not user:
            return {"allowed": False, "reason": "User not found", "suggestions": []}
    ...
    def offer_partial_reveal_options(self, reveal_request: Dict) -> Dict:
        ...
        partial_options = {"available": False, "options": []}
        ...
        if method in ["partial_role", "partial_department"]:
            ...
            partial_options["options"] = [
                {"type": "role", "description": "Reveal role/title only", "available": True},
                {"type": "department", "description": "Reveal department only", "available": True},
            ]
        ...
        # If full reveal requested, offer partial alternatives
            ...
            partial_options["available"] = True
            partial_options["options"] = [
                {"type": "role", "description": "Reveal role only (recommended)", "available": True},
                {"type": "department", "description": "Reveal department only", "available": True},
                {"type": "gradual", "description": "Gradual reveal over time", "available": True},
            ]

        return partial_options

    def process_revoke_anonymity(self, user_id: str, survey_id: Optional[int] = None) -> Dict:
        """
        ...
        try:
            ...
            return {"success": False, "message": f"Error revoking anonymity: {str(e)}"}
    ...
    def _execute_reveal(
        self, user_id: str, method: RevealMethod, reveal_request: Dict, survey_id: Optional[int] = None
    ) -> Dict:
        """
        ...
        reveal_data = {"user_id": user_id, "method": method.value, "timestamp": datetime.utcnow().isoformat()}
        ...
        if method == RevealMethod.FULL:
            ...
            reveal_data["revealed_info"] = {"role_title": user.role_title}

        elif method == RevealMethod.PARTIAL_DEPARTMENT:
            reveal_data["revealed_info"] = {"department": user.department}
        ...
        return {"success": True, "reveal_data": reveal_data}

    def _store_preference(self, user_id: str, preference: str, survey_id: Optional[int] = None):
        """Store identity preference in database"""
        try:
            # Check if preference already exists
            query = self.db.query(SurveyIdentityPreference).filter(SurveyIdentityPreference.user_email == user_id)
            ...
            self.db.commit()
            self.logger.info(f"Stored identity preference: user={user_id}, " f"preference={preference}, survey_id={survey_id}")
        except Exception as e:
            self.db.rollback()
            self.logger.error(f"Error storing preference: {e}")
            raise

    def _load_preference(self, user_id: str, survey_id: Optional[int] = None) -> Optional[str]:
    ...
    def _store_reveal(self, user_id: str, reveal_data: Dict, survey_id: Optional[int] = None):
        ...
        try:
            reveal = SurveyIdentityReveal(
                ...
                next_reveal_date=(
                    datetime.fromisoformat(reveal_data["next_reveal_date"]) if reveal_data.get("next_reveal_date") else None
                ),
            )
            self.db.add(reveal)
            self.db.commit()
            self.logger.info(
                f"Stored identity reveal: user={user_id}, " f"method={reveal_data.get('method')}, survey_id={survey_id}"
            )
        except Exception as e:
            self.db.rollback()
