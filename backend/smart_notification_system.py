...
from backend.database import Assignment, Cycle, EmailNotification, Evaluation, Person
...
class SmartNotificationSystem:
    ...
    def should_notify(self, event_type: str, user_id: str, context: Optional[Dict] = None) -> bool:
        """
    ...
    def get_last_notification(self, user_id: str, event_type: str) -> Optional[EmailNotification]:
        """
    ...
    def _is_duplicate_notification(
        self, user_id: str, event_type: str, context: Optional[Dict], last_notification: EmailNotification
    ) -> bool:
        """
        ...
        if last_notification.related_entity_type and last_notification.related_entity_id:
            if (
                context.get("related_entity_type") == last_notification.related_entity_type
                and context.get("related_entity_id") == last_notification.related_entity_id
            ):
    ...
    def send_notification(self, event_type: str, recipients: List[str], context: Dict, priority: str = "normal") -> Dict:
        """
        ...
        for recipient in recipients:
            ...
            if not self.should_notify(event_type, recipient, context):
                results["skipped"].append({"recipient": recipient, "reason": "User preferences or frequency limits"})
                continue

            # Send notification
            try:
                self.email_service.send_email(to_email=recipient, subject=subject, html_body=message, text_body=message)
    ...
    def schedule_reminder(
        self, event_type: str, recipients: List[str], context: Dict, reminder_date: datetime, repeat: Optional[str] = None
    ) -> Dict:
        """
        ...
        return {"success": True, "scheduled_count": len(scheduled_notifications), "reminder_date": reminder_date.isoformat()}
    ...
    def get_user_behavior_profile(self, user_email: str) -> Dict:
        ...
        completed_evaluations = (
            ...
            .join(Assignment)
            .filter(
                Assignment.rater_email == user_email, Evaluation.status == "submitted", Evaluation.submitted_at.isnot(None)
            )
    ...
    def send_smart_reminder(self, user_email: str, pending_evaluations: List[Dict], cycle_id: int) -> Dict:
        """
        ...
        current_hour = datetime.utcnow().hour
        is_optimal_time = (
            current_hour in behavior["preferred_completion_hours"] or current_hour == behavior["best_reminder_time"]
        )
        ...
        # Send reminder
        return self.send_notification(event_type="smart_reminder", recipients=[user_email], context=context, priority="normal")

    def check_overdue_evaluations(self, cycle_id: int, escalation_days: int = 7) -> Dict:
        """
        ...
        if not cycle or not cycle.end_date:
            return {"success": False, "error": "Cycle not found or no end date"}

        # Get all assignments without submitted evaluations
        pending_assignments = (
            self.db.query(Assignment)
            .outerjoin(Evaluation, Assignment.id == Evaluation.assignment_id)
            .filter(Assignment.cycle_id == cycle_id, Evaluation.id.is_(None))  # No evaluation submitted
            .all()
        )
        ...
        results = {"overdue_count": len(overdue_evaluations), "escalation_count": len(escalation_needed), "alerts_sent": []}

        # Send overdue alerts
        for overdue in overdue_evaluations:
            # Get target person name
            target_person = self.db.query(Person).filter(Person.email == overdue["target_email"]).first()
            ...
            if overdue["days_overdue"] < escalation_days:
                ...
                alert_result = self.send_notification(
                    event_type="evaluation_overdue", recipients=[overdue["rater_email"]], context=context, priority="high"
                )
            else:
                # Escalation alert
                alert_result = self.send_notification(
                ...
                rater_person = self.db.query(Person).filter(Person.email == overdue["rater_email"]).first()

                if rater_person and rater_person.department:
                    # Find department head (simplified - would need proper hierarchy)
                    managers = (
                        self.db.query(Person)
                        .filter(Person.department == rater_person.department, Person.role_title.ilike("%head%"))
                        .all()
                    )
    ...
    def send_due_soon_reminders(self, cycle_id: int, days_before: int = 3) -> Dict:
        """
        ...
        cycle = self.db.query(Cycle).filter(Cycle.id == cycle_id).first()
        if not cycle or not cycle.end_date:
            return {"success": False, "error": "Cycle not found or no deadline"}

        today = datetime.utcnow().date()
        days_remaining = (cycle.end_date - today).days

        if days_remaining > days_before:
            return {"success": False, "reason": f"Too early - {days_remaining} days remaining"}
        ...
        for rater_email, assignments in assignments_by_rater.items():
            ...
            for assignment in assignments:
                ...
                target_names.append(target_person.full_name if target_person else assignment.target_email)
            ...
            reminder_result = self.send_notification(
                event_type="evaluation_due_soon", recipients=[rater_email], context=context, priority="normal"
            )
            ...
            results["details"].append(
                {"rater_email": rater_email, "pending_count": len(assignments), "result": reminder_result}
            )
