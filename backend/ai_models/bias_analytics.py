...
class BiasAnalytics:
        ...
        for rater_id in df['rater_id'].unique():
            ...
            if halo_flag:
                ...
                if variance < 0.5:
                    ...
                    issues.append({
                        'type': 'halo_effect',
                        'severity': halo_flag.get('severity', 'medium'),
    ...
    def generate_bias_heatmap(self, cycle_id: int, df: Optional[pd.DataFrame] = None) -> Dict:
        ...
        if dept_bias.get('status') != 'analyzed':
            return {
                'error': 'Insufficient department data',
                'status': dept_bias.get('status')
            }
    ...
    def analyze_bias_trends(self, cycle_id: int, df: Optional[pd.DataFrame] = None) -> Dict:
        ...
        if not cycle or not cycle.start_date:
            return {
                'error': 'Cycle dates not available',
                'trend_score': 0.0
            }
        ...
        for i, period in enumerate(periods):
            ...
            period_data = df_clean[
                (df_clean['submitted_at'] >= period_start) &
                (df_clean['submitted_at'] < period_end)
            ]
    ...
    def generate_bias_remediation_plan(self, cycle_id: int, df: Optional[pd.DataFrame] = None) -> Dict:
        ...
        low_fairness_raters = [
            (rater_id, data) for rater_id, data in rater_scores.items()
            if data.get('fairness_score', 1.0) < 0.7
        ]
        ...
        biased_departments = [
            (dept, data) for dept, data in departments.items()
            if abs(data.get('bias_score', 0)) > 0.3
        ]
        ...
        for dept, data in biased_departments:
            ...
            intervention['recommendations'].append(
                "Conduct department-wide calibration session"
            )
            intervention['recommendations'].append(
                "Review evaluation distribution and ensure balanced assignments"
            )
        ...
        if overall_bias_score > 0.5:
            ...
            remediation_plan['systemic_recommendations'].extend([
                "Maintain current evaluation standards",
    ...
    def _generate_summary(self, rater_scores: Dict, heat_map: Dict, trends: Dict) -> Dict:
        ...
        # Department summary
        dept_bias_scores = [
            abs(data.get('bias_score', 0))
            for data in heat_map.get('departments', {}).values()
        ]
