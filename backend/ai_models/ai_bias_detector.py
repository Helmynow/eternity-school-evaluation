...
class AIBiasDetector:
    ...
    def analyze_evaluation(self, evaluation: Evaluation) -> Dict:
        ...
        if not evaluation:
            return {
                'status': 'error',
                'message': 'Evaluation object required'
            }
        ...
        similarity_analysis = self._analyze_similarity_bias(
            evaluation, assignment, rater_evaluations
        )
        
        harshness_analysis = self._analyze_harshness_bias(
            evaluation, assignment, target_evaluations
        )
        
        centrality_analysis = self._analyze_centrality_bias(
            evaluation, assignment, rater_evaluations
        )
        
        context_analysis = self._analyze_context_bias(
            evaluation, assignment, target_evaluations
        )
    ...
    def _analyze_similarity_bias(
        ...
        rater_evaluations: List[Evaluation]
    ) -> Dict:
        """
    ...
    def _analyze_context_bias(
        ...
        for eval_obj in target_evaluations:
            eval_assignment = self.db.query(Assignment).filter(
                Assignment.id == eval_obj.assignment_id
            ).first()
            if eval_assignment:
