...
class AcademicAdminScoring:
    ...
    def calculate_batch_scores(
        ...
        if target_emails:
            people = self.db.query(Person).filter(
                Person.email.in_(target_emails),
                Person.active == True
            ).all()
        else:
    ...
    def compare_academic_vs_admin(
        ...
        academic_scores = self.calculate_batch_scores(
            cycle_id=cycle_id,
            staff_type='academic'
        )
        
        admin_scores = self.calculate_batch_scores(
            cycle_id=cycle_id,
            staff_type='admin'
        )
        ...
        recommendations = self._generate_recommendations(
            academic_stats, admin_stats, differences
        )
    ...
    def _generate_recommendations(
        ...
        if mean_diff > 0.5:
            recommendations.append(
                f"Significant difference in mean scores: {mean_diff:.2f} points. "
                "Review weight matrices to ensure fairness."
            )
        ...
        if academic_stats['std_weighted'] > 1.5 or admin_stats['std_weighted'] > 1.5:
            recommendations.append(
                "High variance in scores detected. Review evaluation criteria and rater training."
            )
        
        # Check for fairness
        if mean_diff < 0.2 and academic_stats['count'] > 0 and admin_stats['count'] > 0:
            recommendations.append(
                "Scores are relatively balanced between academic and admin staff. "
                "Current weight matrices appear fair."
            )
        
        if not recommendations:
            recommendations.append(
                "No significant issues detected. Continue monitoring scores for fairness."
            )
    ...
    def validate_evaluations(
        ...
        staff_people = [
            p for p in people
            if self.get_staff_type(p).lower() == staff_type.lower()
        ]
        ...
        for person in staff_people:
            ...
            evaluations = (
                ...
                .filter(
                    Assignment.cycle_id == cycle_id,
                    Assignment.target_email == person.email,
                    Evaluation.status == 'submitted'
                )
                .all()
            )
            ...
            for context, max_count in max_reqs.items():
                ...
                if actual > max_count:
                    warnings.append(
                        f"{person.email} ({staff_type}): {actual} {context} evaluation(s), "
                        f"maximum {max_count} recommended"
                    )
    ...
    def export_scores_to_dict(
        self,
        scores: List[StaffTypeScore]
    ) -> List[Dict]:
