...
class Complete360BiasDetection:
            ...
            for rater_email, data in rater_bias.items():
                ...
                if bias < -0.5:
                    harsh_raters.append({
                        'rater': rater_email,
                        'bias': bias,
                        'mean_rating': data.get('mean_rating', 0)
                    })
                elif bias > 0.5:
                    lenient_raters.append({
                        'rater': rater_email,
                        'bias': bias,
                        'mean_rating': data.get('mean_rating', 0)
                    })
            ...
            if reciprocal_result.get('status') == 'analyzed':
                ...
                if len(pairs) > 0:
                    findings.append(BiasFinding(
                        ...
                        affected_raters=list(set(p.get('person_a') for p in pairs) | set(p.get('person_b') for p in pairs)),
                        affected_targets=[],
    ...
    def _generate_recommendations(self, findings: List[BiasFinding], context_coverage: Dict) -> List[str]:
        ...
        if finding_types.get('role_bias', 0) > 0:
            recommendations.append(
                "Implement rater calibration training to reduce role-based bias"
            )
        
        if finding_types.get('harshness_bias', 0) > 0 or finding_types.get('leniency_bias', 0) > 0:
            recommendations.append(
                "Provide individual feedback and training to raters showing harshness or leniency bias"
            )
        
        if finding_types.get('similarity_bias_halo', 0) > 0:
            recommendations.append(
                "Train raters on differentiated evaluation to reduce halo effect"
            )
        
        if finding_types.get('temporal_bias', 0) > 0:
            recommendations.append(
                "Implement measures to reduce recency bias in evaluations"
            )
