...
from backend.database import Assignment, Cycle, EOMCategory, EOMCycle, EOMNominee, EOMVoter, Person, StaffSegment, WeightMatrix
...
class BulkImporter:
    ...
    def import_staff_from_excel(self, file_path: str) -> Dict:
        ...
        try:
            ...
            return {"imported": imported, "updated": updated, "errors": errors[:10]}
        except Exception as e:
            return {"error": str(e)}
    ...
    def import_eom_voters_from_excel(self, file_path: str, cycle_id: int, month: int, year: int) -> Dict:
        ...
        try:
            ...
            eom_cycle = (
                self.db.query(EOMCycle)
                .filter(EOMCycle.cycle_id == cycle_id, EOMCycle.month == month, EOMCycle.year == year)
                .first()
            )

            if not eom_cycle:
                eom_cycle = EOMCycle(cycle_id=cycle_id, month=month, year=year, status="draft")
            ...
            for _, row in df.iterrows():
                try:
                    ...
                    existing = (
                        ...
                        .filter(EOMVoter.eom_cycle_id == eom_cycle.id, EOMVoter.voter_email == email)
                        .first()
                    )

                    if not existing:
                        voter = EOMVoter(eom_cycle_id=eom_cycle.id, voter_email=email)
                        self.db.add(voter)
                        imported += 1
                except Exception as e:
                    errors.append(f"Row {_ + 2}: {str(e)}")
            ...
            return {"imported": imported, "eom_cycle_id": eom_cycle.id, "errors": errors[:10]}
        except Exception as e:
            return {"error": str(e)}
    ...
    def import_eom_candidates_from_excel(self, file_path: str) -> Dict:
        ...
        try:
            ...
            return {"candidates": candidates, "count": len(candidates), "errors": errors[:10]}
        except Exception as e:
            return {"error": str(e)}
    ...
    def import_weight_matrix_from_excel(self, file_path: str, cycle_id: int) -> Dict:
        ...
        try:
            ...
            existing = (
                self.db.query(WeightMatrix).filter(WeightMatrix.cycle_id == cycle_id, WeightMatrix.is_active == True).first()
            )
            ...
            if existing:
                ...
                weight_matrix = WeightMatrix(
                    cycle_id=cycle_id, name=f"Weight Matrix for Cycle {cycle_id}", matrix_config=matrix_config, is_active=True
                )
