...
class WeightMatrixHandler:
    ...
    def load_evaluations(self) -> List[EvaluationScore]:
        ...
        evaluations = (
            ...
            .filter(
                Assignment.cycle_id == self.cycle_id,
        ...
        for eval_obj, assignment in evaluations:
            ...
            final_weight = base_weight * (assignment.weight if assignment.weight else 1.0)
            
            # Calculate weighted score
            weighted_score = eval_obj.rating * final_weight
    ...
    def calculate_final_scores(self, target_email: Optional[str] = None) -> Dict:
        ...
        scores = (
            [s for s in self._evaluation_scores if s.target_email == target_email]
            if target_email
            else self._evaluation_scores
        )
    ...
    def validate_evaluations(self, target_email: Optional[str] = None) -> ValidationResult:
        ...
        for target, group_counts in target_evaluations.items():
            for target_group, count in group_counts.items():
                ...
                if count < min_eval:
                    errors.append(
                        f"Target {target} ({target_group}) has {count} evaluations, "
                        f"minimum required is {min_eval}"
                    )
                elif count > max_eval:
                    warnings.append(
                        f"Target {target} ({target_group}) has {count} evaluations, "
                        f"maximum recommended is {max_eval}"
                    )
        ...
        for target, score_list in target_scores_map.items():
            ...
            if target_group in ['academic', 'admin']:
                ...
                if not contexts.intersection(required_contexts):
                    warnings.append(
                        f"Target {target} ({target_group}) should have at least "
                        f"one evaluation from CEO, P&C, or QA"
                    )
