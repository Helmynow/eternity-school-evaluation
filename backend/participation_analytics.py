...
from backend.database import Assignment, Cycle, EOMNominee, EOMVoter, Evaluation, Person
...
class ParticipationAnalytics:
    ...
    def analyze_participation(self, cycle: Cycle) -> Dict:
        ...
        assignments = self.db.query(Assignment).filter(Assignment.cycle_id == cycle.id).all()
    ...
    def _calculate_participation_rates(self, cycle: Cycle, assignments: List[Assignment]) -> Dict:
        ...
        for assignment in assignments:
            ...
            evaluation = self.db.query(Evaluation).filter(Evaluation.assignment_id == assignment.id).first()
        ...
        overall_participation_rate = (
            ((total_completed + total_in_progress) / total_assignments * 100) if total_assignments > 0 else 0
        )
    ...
    def _analyze_engagement_trends(self, cycle: Cycle) -> Dict:
        ...
        evaluations = (
            ...
            .join(Assignment)
            .filter(Assignment.cycle_id == cycle.id, Evaluation.status == "submitted", Evaluation.submitted_at.isnot(None))
            .order_by(Evaluation.submitted_at)
            .all()
        )

        if not evaluations:
            return {"status": "insufficient_data", "message": "No submitted evaluations with timestamps found"}
        ...
        if len(sorted_dates) < 2:
            return {"status": "insufficient_data", "message": "Need at least 2 days of data for trend analysis"}

        # Calculate daily rates
        daily_data = [{"date": str(date), "count": daily_submissions[date]} for date in sorted_dates]
        ...
        for date in sorted_dates:
            ...
            cumulative_data.append({"date": str(date), "cumulative": cumulative})
        ...
        if first_half_days > 0 and second_half_days > 0:
            ...
            trend_direction = (
                ...
                else "decreasing" if second_half_avg < first_half_avg else "stable"
            )
    ...
    def _detect_outliers(self, participation_rates: Dict) -> Dict:
        ...
        if dept_rates:
            dept_participation_rates = [(dept, stats["participation_rate"]) for dept, stats in dept_rates.items()]
        ...
        if segment_rates:
            segment_participation_rates = [(segment, stats["participation_rate"]) for segment, stats in segment_rates.items()]
    ...
    def _predict_future_participation(self, current_cycle: Cycle) -> Dict:
        ...
        for hist_cycle in historical_cycles:
            assignments = self.db.query(Assignment).filter(Assignment.cycle_id == hist_cycle.id).all()

            if assignments:
                evaluations = (
                    self.db.query(Evaluation)
                    .join(Assignment)
                    .filter(Assignment.cycle_id == hist_cycle.id, Evaluation.status == "submitted")
                    .count()
                )
        ...
        current_assignments = self.db.query(Assignment).filter(Assignment.cycle_id == current_cycle.id).count()

        current_evaluations = (
            self.db.query(Evaluation)
            .join(Assignment)
            .filter(Assignment.cycle_id == current_cycle.id, Evaluation.status == "submitted")
            .count()
        )
    ...
    def _generate_recommendations(
        self, current_rate: float, predicted_rate: float, historical_rates: List[float]
    ) -> List[str]:
        """
