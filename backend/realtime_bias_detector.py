...
class RealTimeBiasDetector:
    ...
    def analyze_evaluation(self, evaluation_data: Dict, cycle_id: int) -> Dict:
        """
        ...
        if not assignment_id or rating is None:
            return {"status": "error", "message": "Missing required evaluation data"}
        ...
        similarity_bias_score = self.check_halo_effect(assignment, rating, domain_scores, target_evaluations)

        recency_bias = self.check_recency_bias(assignment, rating, submitted_at, cycle_id)

        department_bias = self.check_department_bias(assignment, rating, target_evaluations)
        ...
        fairness_score = self.calculate_fairness_score(similarity_bias_score, recency_bias, department_bias)
    ...
    def check_halo_effect(
        self, assignment: Assignment, rating: float, domain_scores: Dict, target_evaluations: List[Evaluation]
    ) -> Dict:
        """
        ...
        return {"detected": False, "severity": "none", "message": "No significant halo effect detected", "score": 1.0}

    def check_recency_bias(self, assignment: Assignment, rating: float, submitted_at: datetime, cycle_id: int) -> Dict:
        """
        ...
        if not cycle or not cycle.start_date:
            return {"detected": False, "message": "Cycle dates not available", "score": 1.0}
        ...
        if len(all_evaluations) < 10:
            return {"detected": False, "message": "Insufficient data for recency analysis", "score": 1.0}

        # Compare early vs late submissions
        early_evaluations = [
            e for e in all_evaluations if e.submitted_at and (e.submitted_at.date() - cycle.start_date).days < 30
        ]
        late_evaluations = [
            e for e in all_evaluations if e.submitted_at and (e.submitted_at.date() - cycle.start_date).days >= 30
        ]
        ...
        return {"detected": False, "message": "No significant recency bias detected", "score": 1.0}

    def check_department_bias(self, assignment: Assignment, rating: float, target_evaluations: List[Evaluation]) -> Dict:
        """
        ...
        if not target_person:
            return {"detected": False, "message": "Target person not found", "score": 1.0}

        # Get all evaluations grouped by target department/segment
        if len(target_evaluations) < 5:
            return {"detected": False, "message": "Insufficient data for department analysis", "score": 1.0}

        # Group evaluations by target segment
        segment_ratings = defaultdict(list)
        for eval in target_evaluations:
            eval_assignment = self.db.query(Assignment).filter(Assignment.id == eval.assignment_id).first()
            if eval_assignment:
                target_p = self.db.query(Person).filter(Person.email == eval_assignment.target_email).first()
        ...
        return {"detected": False, "message": "No significant department bias detected", "score": 1.0}

    def generate_mitigation_suggestions(self, similarity_bias: Dict, recency_bias: Dict, department_bias: Dict) -> List[str]:
        """
        ...
        if similarity_bias.get("detected"):
            if similarity_bias.get("severity") == "high":
                ...
                suggestions.append("Review domain scores to ensure they reflect different aspects of performance.")
        ...
        if not suggestions:
            suggestions.append("Continue to evaluate based on objective criteria and documented evidence.")

        return suggestions

    def calculate_fairness_score(self, similarity_bias: Dict, recency_bias: Dict, department_bias: Dict) -> Dict:
        """
        ...
        weights = {"similarity": 0.4, "recency": 0.3, "department": 0.3}  # Halo effect is most important
