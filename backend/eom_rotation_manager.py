...
from backend.database import (
    EOMRotationRule, EOMCategory, EOMCycle, Cycle, EOMNominee, EOMWinner
)
...
class EOMRotationManager:
    ...
    def setup_default_rules(
        ...
        for category in categories:
            ...
            existing = self.db.query(EOMRotationRule).filter(
                EOMRotationRule.category == category,
                EOMRotationRule.cycle_id == cycle_id
            ).first()
            
            if existing:
                self.logger.info(
                    f"Rule already exists for {category.value} in cycle {cycle_id}"
                )
                continue
    ...
    def update_rotation_eligibility_flags(
        self,
        eom_cycle_id: int
    ) -> Dict:
        """
        ...
        # Get all nominees
        nominees = self.db.query(EOMNominee).filter(
    ...
    def get_rotation_summary(
        ...
        eom_cycles = self.db.query(EOMCycle).filter(
